...
def run_inference_on_image():
    ...
    # filelist=open('/home/scopeserver/RaidDisk/DeepLearning/mwang/tensorflow/tensorflow/models/image/imagenet/wheretobuy_list.txt','r')
    with open('/home/scopeserver/RaidDisk/DeepLearning/mwang/tensorflow/tensorflow/models/image/imagenet/wheretobuy_list.txt', 'r') as reader:
        box = [line.rstrip() for line in reader]
    ...
    #for i in range(0, 86495):
        ...
        #print (image)
    for image in box:
        currentbox = yaml.load(image)
        fileid=currentbox['id']
        x=int(currentbox['x'])
        y=int(currentbox['y'])
        w=int(currentbox['w'])
        h=int(currentbox['h'])
        classname=currentbox['name']
        filename='/home/scopeserver/RaidDisk/DeepLearning/mwang/tensorflow/Faster-RCNN_TF/data/VOCdevkit/VOC2007/JPEGImages/'+fileid+'.jpg'
        #image = filelist[i]
        if filename.lower().endswith("jpg"):
            ...
            try:
                ...
                cropped_image_filename='/home/scopeserver/RaidDisk/DeepLearning/mwang/data/wheretobuy_parts/'+classname+'/'+str(fileid)+'.jpg'

                if os.path.isfile(cropped_image_filename):
                    cropped_image_filename='/home/scopeserver/RaidDisk/DeepLearning/mwang/data/wheretobuy_parts/'+classname+'/'+str(fileid)+'_'+str(random.randint(0,100))+'.jpg'

                img.save(cropped_image_filename,quality=90)
                ...
                # image_data}) #ADDED
                t1 = time.time()  
                feature_tensor = sess.graph.get_tensor_by_name('pool_3:0')  # ADDED  v1: avgpool0/reshape
                
                feature_set = sess.run(feature_tensor, {'Mul:0': image_4d})  # ADDED   v1: input
                delta = time.time() - t1
                feature_set = np.squeeze(feature_set)  # ADDED
                print(np.size(feature_set))
		        #feature_set= np.array(["%.3f" % x for x in feature_set])
                print(feature_set)  # ADDEDl
                print(delta)
                # files.append(image)
                # features.append(feature_set)
                # os.remove()
                with open("wheretobuy_file.txt", 'a') as f:
                    # for s in image:
                 	f.write(cropped_image_filename + '\n')
                with open("wheretobuy_feature.txt", 'a') as q:
                ...
                # sess.close()
                print(e)
                print("error image: " + fileid)
                # os.remove(image)
                with open("wheretobuy_error.txt", 'a') as t:
                    t.write(image + '\n')
