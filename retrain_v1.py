...
def maybe_download_and_extract():
  ...
  v1_zip=ZipFile(filepath)
  v1_zip.extractall(dest_directory)
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
  ...
  Args:
    ...
    or validation.
    bottleneck_dir: Folder string holding cached files of bottleneck values.
    image: The tensor to feed loaded jpeg data into.
  ...
  if not os.path.exists(bottleneck_path):
    ...
    bottleneck_values = run_bottleneck_on_image(sess, image_4d,
                                                image,
                                                bottleneck_tensor)
...
def cache_bottlenecks(sess, image_lists, image_dir, bottleneck_dir,
  ...
  Args:
    ...
    images.
    bottleneck_dir: Folder string holding cached files of bottleneck values.
    image: Input tensor for jpeg data from file.
...
def get_random_distorted_bottlenecks(
    sess, image_lists, how_many, category, image_dir,
    distorted_image,  bottleneck_tensor):
  ...
  for unused_i in range(how_many):
    ...
    #distorted_image_data = sess.run(distorted_image,
                                    #{input_jpeg_tensor: jpeg_data}) #resized_input_tensor,
    bottleneck = run_bottleneck_on_image(sess, distorted_image,
                                         bottleneck_tensor)
    ground_truth = np.zeros(class_count, dtype=np.float32)
    ground_truth[label_index] = 1.0
    bottlenecks.append(bottleneck)
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if do_distort_images:
      ...
      train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks(
          sess, image_lists, FLAGS.train_batch_size, 'training',
          FLAGS.bottleneck_dir, FLAGS.image_dir, image,
          bottleneck_tensor)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--image_dir',
      type=str,
      default='/home/scopeserver/RaidDisk/fashion_downloader/crawlers/zara_human',
      help='Path to folders of labeled images.'
  )
  parser.add_argument(
      '--output_graph',
      type=str,
      default='/tmp/new_v1.pb',
